...
- Canonical CRM schema: [`docs/fixtures/aitusa-placement-review-crm-envelope-v1.json`](fixtures/aitusa-placement-review-crm-envelope-v1.json) is the cross-repository fixture for `aitusa-crm-event-v1`. It has top-level `placement` (not `placementReview`) with `state` exactly one of `pending`, `in_review`, `confirmed`, `adjusted`, or `additional_review_required`; `finalLevel` appears only after confirm/adjust. Its `correlationId` is the opaque diagnostic attempt/funnel correlation—not the review ID—and the sole employee URL is `/employee/placement-reviews?review=<opaque-review-id>`.
