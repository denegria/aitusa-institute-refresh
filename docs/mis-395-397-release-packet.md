...
- Migration safety: the corrected candidate has not been applied by this lane. Staging identified that the earlier single data-modifying CTE could create review rows without making them visible to its audit/outbox SELECTs. The corrected migration splits the historical backfill into ordered, tool-transaction statement phases: **A** reviews, **B** `placement_review_created` audits, then **C** canonical CRM outbox records. It also names the CRM constructor timestamp argument `event_occurred_at`, preventing a same-named lateral preference column from nulling `occurredAt` when no preference exists. It does not send or call any provider.
- Migration order (staging first, then production only after approval): pause/disable the CRM outbox dispatcher; apply `0006`; run the verification queries below; inspect queued `placement_review_created` entries and only then restore the dispatcher. For a fresh application, phases A/B/C create one review, audit, and outbox record for every claimed result; the final idempotent payload refresh is harmless. For a previously partial application, first recreate `placement_crm_payload` by executing the corrected `DROP FUNCTION IF EXISTS` and `CREATE FUNCTION` statements from `0006` as **separate prepared statements in the same migration-tool transaction** (PostgreSQL requires the drop to rename the timestamp parameter to `event_occurred_at`), then rerun the corrected ordered backfill: phase A conflicts on `result_id`, phase B fills a missing deterministic audit ID, and phase C fills a missing revision-keyed outbox row. Finally run the payload refresh below for undelivered created events. It derives the payload from `placement_reviews` and `crm_outbox.created_at` and updates **only** `payload`; it never changes an idempotency key, correlation ID, review state, or delivery status. Every phase uses deterministic IDs plus `ON CONFLICT`, so it is rerunnable and does not duplicate records.
...
-- Partial-repair check: this must be empty after the corrected function and
-- payload refresh; `occurredAt` is mandatory even without a contact preference.
select id, idempotency_key, status
from crm_outbox
where event_type = 'placement_review_created'
  and status in ('pending', 'retry_wait', 'dead_letter')
  and nullif(payload->>'occurredAt', '') is null;
