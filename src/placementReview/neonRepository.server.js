...
export function createNeonPlacementReviewRepository(database) {
  ...
  return {
    async createReview(input) {
      const result = await database.execute(sql`
        with inserted as (
          ...
          select ${input.eventId}::uuid, id, 'placement_review_created', status, revision, final_level, null, ${input.occurredAt}::timestamptz from selected where replayed = false
    ...
    async transition(input) {
      const existing = await database.execute(sql`select * from placement_review_mutations where review_id = ${input.reviewId}::uuid and mutation_id = ${input.mutationId} limit 1`);
      if (rows(existing)[0]) {
      ...
      const result = await database.execute(sql`
        with updated as (
          ...
          insert into placement_review_events (id, review_id, event_type, status, revision, final_level, actor_account_id, occurred_at)
          select ${input.eventId}::uuid, id, ${placementReviewEventType(input.action)}, status, revision, final_level, ${input.actor.accountId}::uuid, ${input.occurredAt}::timestamptz from updated
        ) select * from updated
      `);
